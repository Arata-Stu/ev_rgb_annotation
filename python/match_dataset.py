...
def parse_timestamp_from_filename(filename):
    ...
    try:
        ...
        if len(parts) == 3:
            ...
            timestamp = datetime.strptime(full_datetime_str, "%Y%m%d_%H%M%S_%f")
            return timestamp
        else:
            print(f"[WARNING] File '{filename}' does not match the expected format.")
            return None
...
def calculate_offsets_with_multiple_cameras(image_dir, start_time):
    """
    サブディレクトリ名をカメラキーとして扱い、画像ファイルのタイムスタンプを収集してオフセットを計算。
    """
    camera_timestamps = {}

    # サブディレクトリごとに処理
    for root, _, files in os.walk(image_dir):
        camera_key = os.path.basename(root)  # サブディレクトリ名をカメラキーとする
    ...
    if not camera_timestamps:
        print(f"[ERROR] No camera timestamps found under {image_dir}")
        return []

    # 全カメラの共通インデックス範囲で平均オフセットを計算
    offsets = []
...
def find_and_process_matching_directories(base_directory, output_directory):
    """
    'events' と 'images' の対応するサブディレクトリを処理し、オフセットを計算して出力。
    """
